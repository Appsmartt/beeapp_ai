...
| `mocks/chats.ts` | Chats y mensajes de conversación (`ChatItem`, `MOCK_CHATS`, `MOCK_CONVERSATION_MESSAGES`), las categorías (`ChatCategory`, `MOCK_CATEGORIES`, `addCategory`, `setChatCategories` y el campo `categoryIds` de cada chat), los miembros del grupo (`GroupMember`, `MOCK_GROUP_MEMBERS`, campo `members` de los chats grupales) y el chat del asistente (`AI_CHAT_ID`, `AI_ASSISTANT_NAME`, `AI_CONVERSATION_MESSAGES`) |
