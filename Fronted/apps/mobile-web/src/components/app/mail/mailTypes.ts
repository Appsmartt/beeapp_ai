...
function getMailSender(
    ...
    const directSenderEmail = normalizeValue(message.sender?.email);
    ...
    const fromEmail = normalizeValue(fromRecipient?.email);
    ...
    const replyToEmail = normalizeValue(replyToRecipient?.email);
...
export function colorFromMailSeed(value: string): string {
    if (!value.trim()) {
...
export function formatMailDetailDate(
    value: string | null,
    ): string {
    if (!value) {
...
export function getMailMessageTimestamp(
    ...
    return message.received_at || message.sent_at || null;
}
...
export function getRecipientLabel(
    ...
    const recipients = email.recipients.filter(
        (recipient) => recipient.recipient_type === 'to',
    );
    ...
    return `Para: ${firstLabel} y ${recipients.length - 1} más`;
}

export function getReplyRecipient(
    email: MailDetailModel,
    ): string {
    const replyToRecipient = email.recipients.find(
        (recipient) => (
...
export function getAttachmentDescription(
    ...
    if (sizeBytes < 1024 * 1024) {
        return `${typeLabel} · ${Math.round(sizeBytes / 1024)} KB`;
    }
