...
import {
    ...
    type MailViewFolder,
    } from '@/components/app/mail/mailTypes';
...
function shouldKeepMessageInCurrentView(
    ...
    switch (activeFolder) {
        ...
        case 'starred':
        return message.is_starred && !isTrashed;
...
export function useMail(
    ...
    const archiveMessage = useCallback(async (
        messageId: string,
    ) => moveMessage(messageId, 'archived'), [moveMessage]);

    const trashMessage = useCallback(async (
        messageId: string,
    ) => moveMessage(messageId, 'trash'), [moveMessage]);

    const restoreMessage = useCallback(async (
        messageId: string,
    ) => moveMessage(messageId, 'inbox'), [moveMessage]);
    ...
    const getMessageById = useCallback(async (
        ...
        getMailMessage(normalizedMessageId),
