...
function getPublicationCopy(profile: CommercialOwnedProfile): {
  label: string;
  color: string;
  Icon: typeof Store;
} {
...
function getVerificationBadgeStyle(profile: CommercialOwnedProfile): {
  ...
  if (
    ...
    profile.verification_status === "requires_correction" ||
...
function getVerificationCopy(profile: CommercialOwnedProfile): string {
  if (profile.verification_status === "verified") {
    return "Verificado";
  }
...
export default function OwnedCommercialProfileCard({
  ...
  const verificationBadge = getVerificationBadgeStyle(profile);
  ...
  const actionBorder = isArchived || isPaused ? "#B7E4C7" : "#F5C2C7";
    ...
    >
      ...
      >
        ...
        >
          ...
          <ChevronRight color="#8D7BA3" size={21} />
        </View>
        ...
        >
          ...
          >
            <ShieldCheck color={verificationBadge.color} size={13} />
        ...
        >
          ...
          >
            {isUpdating ? (
              ...
              <ActionIcon color={actionColor} size={17} />
            )}
            ...
            >
              {isUpdating ? "Actualizando estado…" : actionLabel}
            </Text>
          </TouchableOpacity>
        </View>
      ) : null}
