...
function toSharedMessage(
  ...
  return {
    ...
    sender_id: (
      ...
      || message.sender_identity?.commercial_profile_id
...
function toSharedInboxConversation(
  ...
  const ownParticipant: ChatParticipant = {
    id: `own:${conversation.conversation_id}:${ownIdentityId}`,
    conversation_id: conversation.conversation_id,
    identity_id: ownIdentityId,
    user_id: ownIdentityId,
    role: 'member',
    joined_at: '',
    left_at: null,
...
export async function sendChatMessage(
  ...
  const response = await api.post<{
    ...
    `${conversationPath(conversationId)}messages/`,
    {
      ...
      attachment_file_id: (
        payload.attachment_file_id?.trim()
        || null
      ),
      reference_type: payload.reference_type?.trim() || null,
      reference_id: payload.reference_id?.trim() || null,
...
export async function uploadChatAttachment(
  ...
  payload: {
    sender_identity_id: string;
    message_type: ChatAttachmentMessageType;
    file: {
      uri: string;
      name: string;
      type: string;
  ...
  const normalizedUri = String(payload.file.uri || '').trim();
  const normalizedName = String(payload.file.name || '').trim();
  const normalizedType = String(payload.file.type || '').trim();

  if (!normalizedUri || !normalizedName || !normalizedType) {
    throw new Error(
      'El adjunto de Chat no tiene URI, nombre o tipo MIME válido.',
    );
  }
  ...
  const response = await api.upload<ChatAttachmentUploadResponse>(
...
export async function getChatMessageAttachmentAccess(
  auth: AuthCredentials,
  messageId: string,
  identityId: string,
  download = false,
  ...
  if (!normalizedMessageId || !normalizedIdentityId) {
    throw new Error(
      'No fue posible identificar el adjunto o la identidad de Chat.',
    );
  }
