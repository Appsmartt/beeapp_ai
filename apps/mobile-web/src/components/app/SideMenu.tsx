...
export type MenuOption =
  | 'edit' | 'subscription' | 'integrations' | 'security'
...
export default function SideMenu({ isOpen, onClose }: SideMenuProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* Combined Unified White Container */}
      <div className="relative z-50 flex h-full bg-white shadow-2xl rounded-l-3xl overflow-hidden">
        {/* Left Content Panel */}
        {selectedOption && (
          <div
            className="w-[480px] bg-white h-full flex flex-col overflow-hidden border-r border-neutral-100/50"
            style={{
        ...
        <aside className="w-[340px] bg-white h-full flex flex-col overflow-y-auto">
          ...
          <div className="flex-1 p-4 space-y-6">
            ...
            <div className="space-y-1">
              ...
              <button onClick={() => { handleClose(); router.push('/app/beeservices'); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-normal text-neutral-700 hover:bg-neutral-50 transition-colors text-left">
                <Store className="w-4 h-4 text-neutral-500" /><span>BeeServices</span>
              </button>
            </div>
