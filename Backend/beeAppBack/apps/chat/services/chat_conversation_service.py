...
def get_chat_inbox(
    *,
    ...
    """
    Obtiene el inbox de una identidad propia.

    La identidad se valida antes de revisar caché. El caché almacena
    el payload final, incluidas las URLs de avatar ya enriquecidas,
    para conservar exactamente el contrato HTTP existente.
    """
    try:
        get_owned_chat_identity(
            user_id=user_id,
        ...
        if isinstance(cached_inbox, dict):
            logger.info(
                ...
                extra={
                    ...
                    "limit": int(limit),
                    "has_cursor": (
                        before_last_message_at is not None
                    ),
                },
            )
            return cached_inbox
    ...
    *,
    ...
    for conversation in conversations:
        ...
        try:
            ...
            if (
                not file_record
                or not file_record.get("bucket_id")
